...
def test_guangdong_style_matches_three_part_sum():
    """广东型：与 Q_LT·P_LT + block + 日前偏差 + 实时偏差 一致。"""
    hourly = [
        _make_hour(h, 100, 50, 0.3, 0, 60, None, 0.4, 0.5, 2.0)
        for h in range(24)
    ]
    load = [100.0] * 24
    p_da = [0.4] * 24
    ...
    per = 50 * 0.3 + 2.0 + (60 - 50) * 0.4 + (100 - 60) * 0.5
    assert bd.C_mlt == pytest.approx(24 * (50 * 0.3 + 2.0), rel=0, abs=1e-6)
...
def test_guangxi_style_includes_smooth_and_long_term_diff():
    ...
    hourly = [
        _make_hour(h, 100, 50, 0.3, 0.28, 60, None, 0.4, 0.45, 0.0)
        for h in range(24)
    ]
    load = [100.0] * 24
    p_da = [0.4] * 24
...
def test_cleared_quantity_used_when_configured():
    ...
    hourly = [
        _make_hour(h, 80, 40, 0.3, 0, 50, 70.0, 0.4, 0.5, 0.0)
        for h in range(24)
    ]
    load = [80.0] * 24
    p_da = [0.4] * 24
