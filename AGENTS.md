...
- 先讨论方案再写代码
- 用户是产品经理。编写计划、方案或正式分析时，正文优先呈现架构、业务逻辑、边界、优缺点、风险和决策依据；如确需列出详细代码、测试样例或命令级实现细节，应集中放在文档最后的“技术附件/代码附件”中，避免打断正文阅读。
- 保持代码简洁，不过早抽象
- 修改参数后切换页面需提示未保存
- 重置按钮需确认弹窗
...
## 并行开发规则（两个 Codex 窗口同时工作时）

**如果你是第二个打开此项目的 Codex 窗口，请先提示用户确认文件分工，避免冲突。**
...
**每次对项目进行实质性修改（功能新增、UI调整、参数变更、架构重构等），必须在 `PRD/requirements.md` 中追加对应条目。**

- 格式遵循现有轮次编号（第 N 轮）和 Rxxx 编号体系
- 每条需求用一句话概括变更内容，必要时补充技术细节
- 同一轮次的多个相关变更归为一组，用 `---` 分隔轮次
- 轮次标题格式：`## 第 N 轮（主题，日期）`
